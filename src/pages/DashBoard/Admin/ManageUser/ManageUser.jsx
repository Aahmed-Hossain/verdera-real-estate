  ...
  return (
    <div className="w-11/12 mx-auto">
      <div className="text-center mb-2">
        <h2 className="tex-4xl font-bold text-green-600">Total Users: {users.length} </h2>
      </div>
      <div>
...
<button
  ...
  className={`px-5 py-2 bg-white text-md ${item.role === 'Admin' ? 'cursor-not-allowed' : ''}`}
>
  <span className='relative inline-block px-3 py-1 font-semibold leading-tight'>
    <span
      aria-hidden='true'
      className={`absolute inset-0 ${item.role === 'Admin' ? 'bg-gray-300 cursor-not-allowed' : 'bg-green-300 hover:bg-green-500'} opacity-50 rounded-full`}
    ></span>
